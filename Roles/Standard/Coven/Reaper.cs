...
public class Reaper : CovenBase
{
    ...
    public static void OnAnyoneDead(PlayerControl target)
    {
        ...
        if (Instances.Count > 0 && Main.EnumerateAlivePlayerControls().All(x => x.GetCountTypes() is CountTypes.OutOfGame or CountTypes.None or CountTypes.Crewmate || x.Is(CustomRoles.Reaper)))
